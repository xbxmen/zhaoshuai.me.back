...
function wpjam_topic_user_profile_page(){
	...
	wpjam_fields([
		'nickname'	=> ['title'=>'微信昵称',	'type'=>'view',	'value'=>$wpjam_weixin_user['nickname']],
		'avatar'	=> ['title'=>'微信头像',	'type'=>'view',	'value'=>'<img src="'.str_replace('/132', '/0', $wpjam_weixin_user['headimgurl']).'" width="200" />']
	]);
...
class WPJAM_AdminTopic  {
	...
	public static function get_fields($action_key='', $id=0){
		if($action_key == ''){
			...
			if($wpjam_topic['status']){
				$reply_fields	= ['title'=>'',	'type'=>'view',		'value'=>'<h3>帖子已关闭</h3>'];
			}else{
